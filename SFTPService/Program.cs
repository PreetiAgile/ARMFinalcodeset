    ...
    static void Main(string[] args)
    {
                        ...
                        try
                        {
                            ...
                            if (sftpClient.IsConnected)
                            {
                                ...
                                foreach (var file in files)
                                {
                                    ...
                                    if (file.IsDirectory || file.Name.StartsWith(".") || file.Name == ".." || file.Name == ". " || file.Name.ToUpper().IndexOf($"_FAILURE{fileExtension}") > -1)
                                    {
                                        continue;
                                    }
        ...
        async Task<bool> ProcessFile(string appName, long id, string fileName, string localDirectory, string sourceDirectory, SftpClient sftpClient, string fileSeperator, string username, string mailcontent, string emailto, string trans_id, string mail_subject, string mailfrom, string cc, string bcc)
        {
            ...
            if (conversionStatus)
            {
                ...
                Console.WriteLine($"Conversion failed: {extension}");
        ...
        async Task<bool> ConvertFailureFiles(string localDirectory, string sourceDirectory, string failureFile, string sourceExtension, SftpClient sftpClient, string fileSeperator, string appName, string username, string mailcontent, string emailto, string trans_id, string mail_subject, string mailfrom, string cc, string bcc)
        {
            ...
            {
                Console.WriteLine("failure file path" + failureFilePath);
                Console.WriteLine("source directory is " + sourceDirectory);
                ...
                mailJsonObj.Add("attachments", failureFilePath);
                ...
                if (isConverted && !string.IsNullOrEmpty(convertedFile))
                {
                    if (sftpClient != null)
                    {
                        using (var stream = new FileStream(Path.Combine(localDirectory, convertedFile), FileMode.Open, FileAccess.Read))
                        {
                            Console.WriteLine("file uploaded to" + sourceDirectory +"filename" + convertedFile);
                            sftpClient.UploadFile(stream, Path.Combine(sourceDirectory, convertedFile));
                        }
                    }
                    else
                    {
                        File.Copy(Path.Combine(localDirectory, convertedFile), Path.Combine(sourceDirectory, convertedFile), true);
                    }
                    Console.WriteLine($"Uploaded: {Path.Combine(localDirectory, convertedFile)} to {Path.Combine(sourceDirectory, convertedFile)}");
        ...
        async Task<SQLResult> ImportData(string appName, long id, string localDirectory, string localFileName, string csvFileName)
        {
            ...
            Console.WriteLine("Import data function Result" + table);
            return table;
        }
