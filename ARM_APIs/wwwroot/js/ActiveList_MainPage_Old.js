...
class AxTasks {
    constructor() {
        ...
        this.processRowHtml = `
        <div class="d-flex align-items-center HP-ActiveList-Items" data-process="{{processname}}" data-type="{{tasktype}}" data-taskid="{{taskid}}"  data-transid="{{transid}}" data-keyvalue="{{keyvalue}}">
            <div class="symbol symbol-50px me-5">
                <span class="symbol-label">
                    <img alt="{{processname}}" class=" w-30px" src="../custompages/icons/{{processname}}.png" onerror="this.onerror=null;this.src='../custompages/icons/default.png';">

                </span>
            </div>
            <div class="d-flex align-items-center ">
                <div class="me-5">
                    <a  class="HomeP-ActiveList-Title" title="{{displaytitle}}"  href="javascript:void(0)" onclick="axTasksObj.openProcess('{{processname}}','{{keyfield}}','{{keyvalue}}','{{taskid}}')">{{displaytitle}}</a>
                    <span class="HomeP-ActiveList-Desc">{{displaycontent}}</span>
                    <div class="task-actions-sets d-flex">
                        <!--<a href="javascript:void(0)" title="Approve" class="btn btn-white btn-color-gray-700 btn-active-primary d-inline-flex align-items-center  btn-sm  me-2 ">
                            <span class="material-icons material-icons-style material-icons-2" style="color: #47BE7D;">check_circle</span>
                        </a>
                        <a href="javascript:void(0)" title="Reject" class="btn btn-white btn-color-gray-700 btn-active-primary d-inline-flex align-items-center  me-2  btn-sm"><span class="material-icons material-icons-style material-icons-2" style="color: red;">cancel</span></a>
                        <a href="javascript:void(0)" title="Return" class="btn btn-white btn-color-gray-700 btn-active-primary d-inline-flex align-items-center  me-2  btn-sm"><span class="material-icons material-icons-style material-icons-2" style="color: blueviolet;">reply</span></a>
                        <a href="javascript:void(0)" class="btn btn-white btn-color-gray-700 btn-active-primary d-inline-flex align-items-center  btn-sm  me-2 " title="Send"><span class="material-icons material-icons-style material-icons-2" style="color: darkmagenta;">send</span></a>-->
                        {{task_buttons_html}}
                    </div>
        ...
        <div class="d-flex align-items-center HP-ActiveList-Items" data-process="{{processname}}" data-type="{{tasktype}}" data-taskid="{{taskid}}">
            <div class="symbol symbol-50px me-5">
                <span class="symbol-label">
        ...
        this.taskBtnsHtml.approve = `
            <div class="task-actions-sets d-flex"><a href="javascript:void(0)" title="Approve"
        ...
        this.taskBtnsHtml.reject = `
            <div class="task-actions-sets d-flex"><a href="javascript:void(0)" title="Reject"
        ...
        this.taskBtnsHtml.send = `
            ...
            <a href="javascript:void(0)" title="Send Reasons/Comments" data-popover="SEND{{taskid}}"
                class="btn btn-white btn-color-gray-700d-inline-flex align-items-center  btn-sm  more-comments sendto" data-taskid="{{taskid}}" data-tasktype="{{tasktype}}" data-keyvalue="{{keyvalue}}"
                data-kt-menu-trigger="click" data-kt-menu-placement="bottom" data-kt-menu-flip="top">
                <span class="comments-icons material-icons material-icons-style material-icons-2">more_vert</span>
            </a>
            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bolder w-400px Active_list_popover"
    ...
    init() {
        ...
        this.showTasks('#ActiveList_Container');
        ShowDimmer(false);
    ...
    processTaskInit() {
        ...
        //    document.querySelector("#task-page-title").innerHTML = this.dataSources["Tasks"][0]["displaytitle"];
        //}
        this.showTasks('#process_centerpanel');
        ShowDimmer(false);
        ...
        document.querySelectorAll(".sendto").forEach((item) => {
            KTMenu.getInstance(item).on("kt.menu.dropdown.show", function (item) {
                ...
                if (sendToDropDown != null && sendToDropDown?.classList.contains("data-fetched") == false) {
                    _this.getSendToUsers(itemData.taskid, itemData.tasktype, itemData.keyvalue, sendToDropDown);
                }
            });
        });
    ...
    fetchBulkTasks(name) {
        ...
        if (_this.isAxpertFlutter) {
            url = "../../api/v1/ARMGetBulkActiveTasks";
        }
        this.callAPI(url, data, false, result => {
            if (result.success) {
                let json = JSON.parse(result.response);
    ...
    getSendToUsers(taskId, taskType, keyValue, sendToDropDown) {
        let _this = this;
    ...
    doTaskAction(action, taskId, taskType, elem) {
        ...
        if (!this.isUndefined(window.axProcessObj)) {
            url = "../../aspx/AxPEG.aspx/AxDoTaskAction";
        }
        if (_this.isAxpertFlutter) {
    ...
    showTasks(container) {
        ...
        if (this.dataSources["Tasks"]?.length == undefined || this.dataSources["Tasks"].length == 0) {
            if (this.isUndefined(parent.axProcessObj)) {
                document.querySelector(container).insertAdjacentHTML("beforeend", ` ${this.noRecordsRowHtml} `);
            }
            else {
                document.querySelector(container).insertAdjacentHTML("beforeend", ` ${this.processedRowHtml} `);
            }
        }
        else {
            //document.querySelector(container).innerHTML = "";
            this.dataSources["Tasks"].forEach((rowData, idx) => {
                ...
                switch (rowData.tasktype.toUpperCase()) {
                    case "MAKE":
                        ...
                        break;
                    case "CHECK":
                        if (rowData?.initiator_approval == "F" && rowData.touser == rowData.initiator) {
                ...
                if (container == "#process_centerpanel") {
                    htmlText = htmlText.replaceAll('../custompages/icons', '../../custompages/icons');
                }
                ...
                if (container == "#process_centerpanel") {
                    document.querySelector('.HP-ActiveList-Items').click();
                    document.querySelector('.HP-ActiveList-Items').style["background-color"] = "#fff";
                }
                if (rowData.tasktype.toUpperCase() == "APPROVE") {
            ...
            if (container == "#ActiveList_Container")
    ...
    openBulkApprove() {
        ...
        try {
            ...
            <div class="card-body d-flex ">
                ...
                <div class="w-100">
                    <div class="approval-controls" data-tasktype="Approve">
                        <label class="form-label col-form-label ">Bulk Approval Comments</label>
                        <div class="input-group">
                ...
                <div>
                    <button class="btn btn-primary" type="button" onclick="axTasksObj.doBulkAction('BULKAPPROVE','APPROVE');">Bulk Approve</button>
                </div>
            </div>`;
            modalObj.size = "lg";
        ...
        document.querySelectorAll(`.taskSubrow${taskId}`).forEach((view) => {
            ...
            if (!view.classList.contains("data-fetched")) {
                ...
                if (contentHtml.indexOf('{') > -1 || contentHtml.indexOf('[') > -1) {
                    ...
                    axHtmlObj.parse({ name: taskId, input: view.dataset["displaymcontent"], data: {}, pageVariables: { "taskid": taskId, "tasktype": taskContainer.getAttribute("data-type"), "transid": taskContainer.getAttribute("data-transid"), "keyvalue": taskContainer.getAttribute("data-keyvalue") }, container: `.taskSubrow${taskId} span` });
                }
                else {
